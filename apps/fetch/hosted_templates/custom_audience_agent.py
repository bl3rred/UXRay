...
AGENT_NAME = "uxray_custom_audience_agent"
...
def build_review(correlation_id: str, issue: IssuePacket, round_number: int, rebuttal_request: str | None) -> AudienceReview:
    ...
    custom_weight = 1 if issue.custom_audience else 0
    impact_score = clamp_score(3 + severity_weight + audience_bias + custom_weight)
    priority_score = clamp_score(impact_score + min(len(issue.evidence), 3))
    rationale = (
        f"Custom audience review sees {issue.issue_type.replace('_', ' ')} on {issue.route} "
        f"as a {issue.severity}-severity blocker for {issue.persona}. Evidence counted: {len(issue.evidence)}."
    )
    if issue.custom_audience:
        rationale = f"{rationale} Custom audience context: {issue.custom_audience}."
    if rebuttal_request:
...
def render_specialist_summary(issue: IssuePacket) -> str:
    ...
    return (
        f"Custom audience lens: {review.priority_score}/10 urgency. "
...
def render_role_text() -> str:
    return (
        "I evaluate issues through a custom audience lens, adapting the review to audience-specific context when it is "
        "provided. Use uxray_orchestrator_agent for the full UXRay multi-agent review."
    )
...
agent = Agent()
protocol = Protocol(name="uxray_custom_audience_protocol")
