...
def build_review(correlation_id: str, issue: IssuePacket, round_number: int, rebuttal_request: str | None) -> AudienceReview:
    ...
    rationale = (
        f"First-time visitors see {issue.issue_type.replace('_', ' ')} on {issue.route} "
...
def render_specialist_summary(issue: IssuePacket) -> str:
    ...
    return (
        f"First-time visitor lens: {review.priority_score}/10 urgency. "
...
def render_role_text() -> str:
    return (
        "I evaluate issues through a first-time visitor lens, focusing on clarity, discoverability, "
        "and whether a new user can confidently take the next step. Use uxray_orchestrator_agent "
        "for the full UXRay multi-agent review."
    )
...
agent = Agent()
protocol = Protocol(name="uxray_first_time_visitor_protocol")
