...
def build_review(correlation_id: str, issue: IssuePacket, round_number: int, rebuttal_request: str | None) -> AudienceReview:
    ...
    rationale = (
        f"Trust-sensitive users see {issue.issue_type.replace('_', ' ')} on {issue.route} "
        f"as a {issue.severity}-severity blocker for {issue.persona}. Evidence counted: {len(issue.evidence)}. "
        f"Users will hesitate if the interface feels ambiguous at the point of conversion."
    )
    if rebuttal_request:
...
def render_specialist_summary(issue: IssuePacket) -> str:
    ...
    return (
        f"Trust evaluator lens: {review.priority_score}/10 urgency. "
...
def render_role_text() -> str:
    return (
        "I evaluate issues through a trust and credibility lens, focusing on whether users feel safe and confident "
        "enough to proceed. Use uxray_orchestrator_agent for the full UXRay multi-agent review."
    )
...
agent = Agent()
protocol = Protocol(name="uxray_trust_evaluator_protocol")
